...
getting heads is $0.5+\epsilon$
for some known $\epsilon>0$). Can you determine if you have the fair coin or not? 
...
flip the coin an order of $\frac{1}{\epsilon^2}$
times to reliably figure out if there is a bias. This would require $\text{log}(1/\epsilon^2)$
...
We will need two arbitrary rotation gates, $R_x(\epsilon)$
and $R_x(-\theta)$.
