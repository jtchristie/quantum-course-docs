# Applications of Qubits outside of Speedups

A huge part of the course is seeing how we can design quantum algorithms that will provide a speedup in solving problems over classical computers. However, we can also look at a couple of applications of qubits themselves.
For one, there are infinite possible values for the amplitudes of the $\ket{0}$
and $\ket{1}$
states of a qubit, so we can try to pack a lot of data into a qubit. Note that since measurement only yields a 0 or 1, we can't extract more than one bit of data. But this
...
for some known $\theta>0$). Can you determine if you have the biased coin or not? 

The only way you can figure this out is by repeatedly flipping the coin and keeping track of the number of heads and tails. Statistics tell us that we would have to
flip the coin on an order of $\frac{1}{\theta^2}$
times to reliably figure out if there is a bias. This would require $\log_2(1/\theta^2)$
...
If the coin was fair, then we expect there to be roughly the same number of clockwise and counterclockwise rotation, so that the final state of the qubit
...
and the probability of clockwise rotation is $0.5-\theta$.
...
coin flips achieves an expected rotation of $\frac{\pi}{4\theta^2}\cdot2\theta^2=\frac{\pi}{2}$.
The probability of measuring the qubit as being in the $\ket{1}$
state would then be quite high. 
...
work for the Quantum Bomb Squad and recieve a suspicious package at the office. The package has peepholes at opposite ends through which a qubit can be sent (perhaps a photonic qubit). It also has a  note saying that the box is either empty of contains a bomb which is attached to a measurement device set up behind one of the peepholes. The measurment device will detonate the bomb if it measures 1 from a qubit coming in through the peephole (if it
measures a 0 it will just let the qubit pass all the way through and out the other peephole). Your goal is to detect if there is a bomb or not while minimizing the chances of setting it off.
...
If $\theta$
is a small angle, then the probability of setting off the bomb with this qubit is the probability of measuring $\ket{1}$:
$$|\sin(\theta)|^2\approx \theta^2.$$
Now, hoping you weren't unlucky and detonated the bomb, keep applying $R_x(\theta)$ to the qubit and passing it through the device. If the bomb
was really there, then it would be measuring the qubit each time and (very likely) collapsing it back to the $\ket{0}$
...
which can be made as close to 1 as you want as long as you are willing to decrease parameter $\theta$
and perform more trials.

On the other hand, if there was a never bomb then the qubit would pass through unaffected each time and you would have applied roughly $\pi/(2\theta)$
rotations by angle $\theta$
which would put the qubit in the $\ket{1}$
...
of changing states from the one it was initialized to.
...
communication between two parties, Alice and Bob, that avoids nasty eavesdropper Eve. First, we introduce the idea of **one-time pad encryption**. Suppose Alice wants to send Bob a secrete message which she stored in the binary bit string $m=m_0m_1...m_{n-1}$.
...
The reason it is called one-time pad encrytion is because the secret key, $s$
should be only used once, since otherwise Eve can use cryptographic techniques
on the many encrypted messages she intercepts to begin and decypher the value of $s$.
...
following solution. She randomly generates two random $n$-
bit strings, $x$ and $y$.
She creates an $n$-
qubit state initilized to state $\ket{x}$.
She then alters each qubit in the following way: if $y_i=1$ apply an $H$ gate to qubit $i$.
...
Alice then sends the resulting state to Bob. Bob randomly generates a random $n$-
bit string $z$
...
and
$z=0101$.
Then Alice aplies Hadamard gates to the first two qubits of $\ket{0101}$
resulting in the state 
$$\ket{+}\otimes\ket{-}\otimes\ket{01}=\ket{+-01}.$$
...
Measuring the state might yield different results for $t$
but it will always have the middle two qubits measuring to $\ket{10}$.
Since $y$ and $z$
...
Bob then measures and keeps this bit as an entry of the shared secret key.
...
But when the quantum state is sent, only Alice knows which bits are in the $\ket{0},\ket{1}$
states and which bits are in the $\ket{+},\ket{-}$
states (since this is determined by $y$).
If Eve just naively measures the qubits of the state, all the qubits in the $\ket{+},\ket{-}$
...
gate to remove the superposition, measure the qubit (which will remain unaltered since it is now either $\ket{0}$
or $\ket{1}$),
...
(even though these classically shared values cant be used later on). Eavesdropping alters half the qubits of the transmitted state and there is still a $0.5$ 
...
7. Alice and Bob share some of the bits of $x',t'$. If these bits agree, then the unshared bits of $x',t'$ are probably equal/not eavesdropped and can serve as the shared private key.
