...
The controlled Z gate turns out to be a special controlled gate, since it doesn't matter which qubit is the target and which is the control. This is because the controlled Z gate only applies a phase of -1 when both bits are in the $\ket{1}$ state. This is why the gate is sometimes written as a pair of connected filled dots seen below:
